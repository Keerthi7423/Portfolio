  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex flex-wrap justify-center gap-6">
        {/* Primary Stark CTA */}
        <button className="group relative px-10 py-4 bg-arc-blue/5 border border-arc-blue/30 text-arc-blue font-orbitron text-sm tracking-[0.2em] uppercase transition-all duration-300 overflow-hidden hover:bg-arc-blue/10">
          <span className="relative z-10">Initiate Mission</span>
          
          {/* Holographic background sweep */}
          <div className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 bg-gradient-to-r from-transparent via-arc-blue/20 to-transparent skew-x-12" />
          
          {/* Corner decorations */}
          <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-arc-blue transition-all group-hover:w-4 group-hover:h-4" />
          <div className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-arc-blue transition-all group-hover:w-4 group-hover:h-4" />
        </button>

        {/* Secondary Dossier CTA */}
        <button className="group relative px-10 py-4 bg-marvel-red/5 border border-marvel-red/30 text-marvel-red font-orbitron text-sm tracking-[0.2em] uppercase transition-all duration-300 overflow-hidden hover:bg-marvel-red/10">
          <span className="relative z-10">Access Dossier</span>
          
          {/* Holographic background sweep */}
          <div className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 bg-gradient-to-r from-transparent via-marvel-red/20 to-transparent skew-x-12" />
          
          {/* Corner decorations */}
          <div className="absolute top-0 right-0 w-2 h-2 border-t border-r border-marvel-red transition-all group-hover:w-4 group-hover:h-4" />
          <div className="absolute bottom-0 left-0 w-2 h-2 border-b border-l border-marvel-red transition-all group-hover:w-4 group-hover:h-4" />
        </button>
      </motion.div>
